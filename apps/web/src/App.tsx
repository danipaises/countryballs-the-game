...
function MatchScreen({ admission, leave }: { admission: CloudAdmission; leave(): void }) {
  const client = useMemo(() => gameServerProvider.connect(admission), [admission]);
  const [snapshot, setSnapshot] = useState<ArenaSnapshot | null>(null); const [roster, setRoster] = useState<CloudRosterEntry[]>([]);
  const [connection, setConnection] = useState('Conectando…'); const [error, setError] = useState('');
  useEffect(() => { const unsubscribe = client.subscribe(event => {
    if (event.type === 'connected') { setConnection('Online'); setError(''); }
    if (event.type === 'state') setSnapshot(event.snapshot); if (event.type === 'roster') setRoster(event.roster);
    if (event.type === 'error') setError(event.message); if (event.type === 'closed') setConnection(event.message);
  }); client.connect(); return () => { unsubscribe(); client.close(); }; }, [client]);
  const local = snapshot?.players.find(player => player[0] === admission.playerId); const remaining = snapshot ? Math.ceil(snapshot.remainingTicks / 30) : 0;
  const clock = `${Math.floor(remaining / 60)}:${String(remaining % 60).padStart(2, '0')}`;
  const arenaNames = { 'orbital-station': 'Estação Orbital', 'street-court': 'Quadra de Rua', 'tropical-island': 'Ilha Tropical' } as const;
  const arenaName = snapshot ? arenaNames[snapshot.arenaId] : 'Estação Orbital'; const isReady = Boolean(local?.[6]);
  return <div className="match-page">
    <header className="match-header"><button className="back-button" onClick={leave} aria-label="Sair da partida">←</button><div className="match-title"><b>{arenaName}</b><span><i /> {connection}</span></div><div className="level-chip">ARENA {snapshot?.arenaId === 'street-court' ? '3' : snapshot?.arenaId === 'tropical-island' ? '2' : '1'}</div>{admission.room.code && <button className="room-code" onClick={() => void navigator.clipboard?.writeText(admission.room.code ?? '')}><small>CÓDIGO</small><strong>{admission.room.code}</strong></button>}</header>
    <div className="match-layout"><main className="game-column">
      <div className="game-hud"><div><small>VIDA</small><strong>♥ {local?.[3] ?? 100}</strong></div><div className="round-state"><small>{snapshot?.phase === 'running' ? 'TEMPO' : 'PARTIDA'}</small><strong>{snapshot?.phase === 'running' ? clock : snapshot?.phase === 'ended' ? 'FIM' : 'LOBBY'}</strong></div><div><small>PONTOS</small><strong>★ {local?.[4] ?? 0}</strong></div></div>
      <Suspense fallback={<div className="game-loading">Preparando arena…</div>}><ArenaGame client={client} snapshot={snapshot} roster={roster} /></Suspense>
      {snapshot?.phase === 'lobby' && <div className="ready-overlay"><div><b>{isReady ? 'Você está pronto!' : roster.length < 2 ? 'Esperando outro jogador' : 'A batalha vai começar'}</b><span>{isReady ? 'Aguardando os outros jogadores…' : roster.length < 2 ? 'Compartilhe o código da sala' : 'Confirme para entrar na arena'}</span></div><button disabled={isReady} onClick={() => client.command('ready')}>{isReady ? 'PRONTO ✓' : 'ESTOU PRONTO'}</button></div>}
      {snapshot?.phase === 'ended' && <div className="ready-overlay result-overlay"><div><b>🏆 {snapshot.winnerId ? `${roster.find(player => player.playerId === snapshot.winnerId)?.name ?? 'Jogador'} venceu!` : 'Fim de jogo'}</b><span>A próxima revanche será em {snapshot.arenaId === 'orbital-station' ? 'Quadra de Rua' : snapshot.arenaId === 'street-court' ? 'Ilha Tropical' : 'Estação Orbital'}.</span></div><button disabled={isReady} onClick={() => client.command('rematch')}>{isReady ? 'AGUARDANDO…' : 'REVANCHE'}</button></div>}
      {error && <div className="match-error" role="alert"><b>Conexão interrompida</b><span>{error}</span><button onClick={() => setError('')}>×</button></div>}
    </main>
    <aside className="scoreboard"><div className="score-title"><div><small>SALA</small><h2>{admission.room.name}</h2></div><span>{roster.length}/{admission.room.maxPlayers}</span></div>{roster.map(player => { const state = snapshot?.players.find(value => value[0] === player.playerId); return <div className="player-row" key={player.playerId}><span className={`flag-token flag-${player.country.toLowerCase()}`}>{player.country}</span><div><b>{player.name}{player.playerId === admission.playerId ? ' · VOCÊ' : ''}</b><small>{state?.[6] ? 'Pronto' : snapshot?.phase === 'running' ? `${state?.[3] ?? 100} HP` : 'No lobby'}</small></div><strong>{state?.[4] ?? 0}</strong></div>; })}<div className="arena-route"><small>ROTA DAS ARENAS</small><b>1 Orbital</b><span>3 Rua</span><span>2 Ilha</span></div><p className="controls-hint">WASD ou setas para mover<br />Espaço para atacar</p></aside>
    </div>
  </div>;
}
